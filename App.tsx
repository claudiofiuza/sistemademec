  ...
  if (isBooting) {
    return (
      <div className="h-screen w-full bg-[#0a0c10] flex flex-col items-center justify-center p-10">
        <div className="w-16 h-16 rounded-2xl bg-emerald-500 flex items-center justify-center text-slate-900 text-3xl animate-pulse">
          <i className="fa-solid fa-car"></i>
        </div>
        <p className="text-[10px] font-black text-emerald-500 uppercase tracking-[0.5em] mt-8 animate-pulse">Sincronizando Supabase...</p>
      </div>
    );
  }
    ...
    <HashRouter>
      <div className="flex h-screen w-full bg-[#0a0c10] text-slate-100 overflow-hidden font-['Inter']">
        ...
        <main className="flex-1 overflow-auto bg-[#0a0c10]">
          <Routes>
            <Route path="/login" element={currentUser ? <Navigate to="/" /> : <LoginPage users={globalUsers} workshops={workshops} onLogin={handleLogin} settings={DEFAULT_SETTINGS} />} />
...
const Sidebar: React.FC<{ user: User, workshop: Workshop | null, activeWorkshopId: string | null, dbStatus: string, onLogout: () => void, onResetContext: () => void, isSyncing: boolean, onManualSync: () => void }> = ({ user, workshop, activeWorkshopId, dbStatus, onLogout, onResetContext, isSyncing, onManualSync }) => {
  ...
  return (
    <div className="w-64 bg-[#11141a] border-r border-slate-800/50 flex flex-col shrink-0">
      <div className="p-6">
        <div className="flex items-center space-x-3 mb-8">
          ...
          <div className="overflow-hidden text-left">
            ...
            <p className="text-[8px] text-emerald-500 font-black uppercase tracking-widest mt-1 opacity-60">DATABASE CONNECTED</p>
          </div>
        </div>
      ...
      <nav className="flex-1 px-3 space-y-1 overflow-y-auto custom-scrollbar">
        {isSuperAdmin && <NavLink to="/central" icon="fa-microchip" label="Central Control" />}
      ...
      <div className="p-4 border-t border-slate-800/50">
        ...
        <div className="bg-[#0a0c10] rounded-xl p-3 border border-slate-800">
          <div className="flex items-center justify-between mb-3 px-1">
             <span className="text-[8px] font-bold text-slate-500 uppercase tracking-widest">Supabase</span>
          ...
          <div className="flex items-center space-x-2">
            ...
            <button onClick={onLogout} className="p-2 text-slate-500 hover:text-red-500 transition-colors"><i className="fa-solid fa-power-off text-[10px]"></i></button>
          </div>
        </div>
      </div>
    </div>
