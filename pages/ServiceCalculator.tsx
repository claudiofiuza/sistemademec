...
import { User, Part, ServiceRecord, AppSettings } from '../types';
...
const ServiceCalculator: React.FC<CalculatorProps> = ({ user, parts, settings, onSave }) => {
  ...
  const freelanceFee = useMemo(() => (inGameCost || 0) * (settings?.freelanceMultiplier || 1), [inGameCost, settings.freelanceMultiplier]);
  const subtotalParts = useMemo(() => (selectedPartIds || []).reduce((sum, partId) => {
    const part = (parts || []).find(p => p.id === partId);
    return sum + (part ? part.price : 0);
  }, 0), [selectedPartIds, parts]);

  // Taxa é calculada sobre a mão de obra para dívida do mecânico, mas não altera o total cobrado
  const taxAmount = useMemo(() => freelanceFee * (settings?.taxRate || 0), [freelanceFee, settings.taxRate]);
  const finalTotalForCustomer = subtotalParts + freelanceFee;
  ...
  const handleSave = async () => {
    ...
    const record: ServiceRecord = {
      ...
      inGameCost,
      freelanceFee,
      totalAmount: finalTotalForCustomer,
      tax: taxAmount, // Imposto rastreado para o RH
      finalPrice: finalTotalForCustomer,
    ...
    setTimeout(() => {
      ...
      setCustomerName(''); setCustomerId(''); setAuthorizedBy(''); setInGameCost(0); setSelectedPartIds([]); setScreenshot(null);
    }, 3000);
  };

  return (
    <div className="p-8 max-w-6xl mx-auto pb-24 fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 text-left">
                ...
                <div key={cat} className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {catParts.map(part => {
                      ...
                      return (
                        <div key={part.id} onClick={() => togglePart(part.id)} className={`p-5 border-2 rounded-2xl cursor-pointer transition-all flex items-center justify-between group ${isSelected ? 'bg-primary border-primary shadow-primary-glow' : 'bg-slate-900 border-slate-800 hover:border-slate-700'}`}>
                          <div>
        ...
        <div className="space-y-6">
          <section className="bg-slate-900 border border-slate-800 p-8 rounded-[2.5rem] sticky top-8 shadow-2xl overflow-hidden text-left">
            ...
            <div className="border-t border-slate-800 pt-6">
              <div className="flex justify-between text-4xl font-black text-primary">
                ...
                <span className="font-mono tracking-tighter">{settings.currencySymbol}{finalTotalForCustomer.toLocaleString()}</span>
              </div>
              <p className="text-[9px] text-slate-600 uppercase font-black text-center mt-6 italic tracking-widest">PAGAMENTO INTEGRAL NO ATO DO SERVIÇO</p>
            </div>
            ...
            <div className="mt-10 space-y-4">
              ...
              <button 
                onClick={handleSave}
                disabled={!customerName || !customerId}
                className="w-full bg-primary hover:opacity-90 text-slate-950 font-black py-5 rounded-2xl shadow-primary-glow disabled:opacity-30 transition-all uppercase tracking-widest text-xs"
              >
