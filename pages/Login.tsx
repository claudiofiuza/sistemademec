    ...
    <div className="min-h-screen flex items-center justify-center p-6 bg-[#0a0c10] relative overflow-hidden">
      <div className="max-w-md w-full relative z-10 animate-in fade-in zoom-in-95 duration-500">
        <div className="text-center mb-10">
          <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-emerald-500 text-slate-950 text-3xl mb-4 shadow-xl shadow-emerald-500/10">
            <i className="fa-solid fa-shield-halved"></i>
          </div>
          <h1 className="text-2xl font-black text-white tracking-tighter uppercase italic leading-none">LSC PRO V5</h1>
          ...
          {step === 'credentials' ? (
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-2 text-left">
                ...
                <input
                  ...
                  className="w-full bg-[#0a0c10] border border-slate-800 rounded-xl py-4 px-6 focus:outline-none focus:border-emerald-500/50 transition-all text-white placeholder-slate-800 font-bold uppercase tracking-widest"
              ...
              <div className="space-y-2 text-left">
                ...
                <input
                  ...
                  className="w-full bg-[#0a0c10] border border-slate-800 rounded-xl py-4 px-6 focus:outline-none focus:border-emerald-500/50 transition-all text-white placeholder-slate-800 font-mono"
                  placeholder="••••••••"
                  required
                />
              </div>
              ...
              <button type="submit" className="w-full bg-emerald-500 hover:opacity-90 text-slate-950 font-black py-4 rounded-xl transition-all shadow-lg shadow-emerald-500/10 uppercase tracking-widest text-xs italic">
                ESTABELECER CONEXÃO
              </button>
            </form>
          ) : (
            <div className="space-y-6">
              ...
              <div className="space-y-3 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                {workshops.map(ws => (
                  <button key={ws.id} onClick={() => onLogin(tempUser!, ws.id)} className="w-full bg-[#0a0c10] border border-slate-800 p-4 rounded-xl text-left hover:border-emerald-500/50 hover:bg-emerald-500/5 transition-all flex items-center justify-between group">
                    <span className="font-bold text-white uppercase italic text-sm">{ws.name}</span>
                    <i className="fa-solid fa-chevron-right text-slate-800 group-hover:text-emerald-500 text-xs transition-all"></i>
                  </button>
                ))}
